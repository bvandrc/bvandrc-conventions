...
- **Branches**
  - **Naming**: Name work branches `<type>/<slug>`, where `<type>` is a
  ...
  - **Renaming an assigned branch**: The naming rule above overrides the
    branch name a session is assigned by default — if you were given one,
    rename it before the first push.
